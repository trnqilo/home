...
#if canImport(PlaygroundFFI)
import PlaygroundFFI
#endif
...
fileprivate extension RustBuffer {
    ...
    static func from(_ ptr: UnsafeBufferPointer<UInt8>) -> RustBuffer {
        try! rustCall { ffi_Playground_c83d_rustbuffer_from_bytes(ForeignBytes(bufferPointer: ptr), $0) }
    }

    // Frees the buffer in place.
    // The buffer must not be used after this is called.
    func deallocate() {
        try! rustCall { ffi_Playground_c83d_rustbuffer_free(self, $0) }
    }
}
...
public func createPlayground( name: String )  -> String {
    let _retval = try!
    
    
    rustCall() {
    
    Playground_c83d_create_playground(name.lower() , $0)
}
 ...
 */
public enum PlaygroundLifecycle {
    /**
