...
if [[ "$whereami" == 'android' ]]; then
  export packages='build-essential python3 nodejs openjdk-21 which openssh screen bat rsync proot-distro'
  alias prootsh='proot-distro sh'
  function proots { if [[ ! "$1" ]]; then proot-distro list; else proot-distro $@; fi; }
elif [[ "$whereami" == 'darwin' ]]; then
  function app { local app="$@"; open -a "$app"; }
  export packages='bash zsh coreutils gnu-sed watch bash-completion zsh-completion gpg qemu bat lolcat openssl screen brew-gem grep lazy-docker'
...
alias lopro='mkdir -p "$HOME/.local" && edi "$HOME/.local/profile";' profo='vim "$HOME/.profo" && source "$HOME/.profo"'
...
alias cons='mkdir -p "$HOME/Contexts" && cd "$HOME/Contexts" && edi .'
