...
function ctx { (mkdir -p "$CTX" && cd "$CTX" && edi . $(while [[ "$1" ]]; do printf " ${1//.md/}.md"; shift; done)); }
...
if [[ "$whereami" == 'android' ]]; then
  ...
  export packages='bash zsh coreutils gnu-sed watch bash-completion zsh-completion gpg qemu bat lolcat openssl screen brew-gem grep lazydocker wakeonlan'
  ...
  alias glitchyscreenshare='sudo killall RFBEventHelperd'
  alias glitchyaudio='sudo pkill coreaudiod bluetoothd'
...
alias bot='${bot:-codex}' ai=bot bb=bot
